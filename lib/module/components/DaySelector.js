...
import { getParsedDate, getMonthDays, getWeekdaysMin, getDaysInMonth, areDatesOnSameDay, isDateBetween, getDate, getFormated } from '../utils';
const DaySelector = () => {
  var _getWeekdaysMin;
  const {
    mode,
  ...
  const daysGrid = useMemo(() => {
    ...
    return getMonthDays(currentDate, displayFullDays, minDate, maxDate, disabledDates, firstDayOfWeek).map((day, index) => {
      if (day) {
  ...
  [mode, month, year, displayFullDays, firstDayOfWeek, minDate, maxDate, date, disabledDates, startDate, endDate, dates]);
